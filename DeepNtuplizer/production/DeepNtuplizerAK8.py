...
process.options = cms.untracked.PSet(
    allowUnscheduled=cms.untracked.bool(True), wantSummary=cms.untracked.bool(False)
)
...
process.load(
    "DeepNTuples.DeepNtuplizer.samples.QCD_Pt_600to800Phase1_cfg"
)  # default input
...
if (
    options.inputScript != ""
    and options.inputScript != "DeepNTuples.DeepNtuplizer.samples.TTJetsPhase1_cfg"
):
...
process.source.fileNames = process.source.fileNames[
    jobNumber:numberOfFiles:numberOfJobs
]
if options.nJobs > 1:
...
if hasattr(process, "updatedPatJetsTransientCorrectedSoftDropSubjetsDeepFlavour"):
    process.updatedPatJetsTransientCorrectedSoftDropSubjetsDeepFlavour.addTagInfos = (
        cms.bool(True)
    )
    process.updatedPatJetsTransientCorrectedSoftDropSubjetsDeepFlavour.addBTagInfo = (
        cms.bool(True)
    )
    print ">>>Adding ValueMaps for updatedPatJetsTransientCorrectedSoftDropSubjetsDeepFlavour"
else:
    raise ValueError(
        "I could not find updatedPatJetsTransientCorrectedSoftDropSubjetsDeepFlavour to embed the tagInfos, please check the cfg"
    )
...
if hasattr(process, "updatedPatJetsTransientCorrectedAK8DeepFlavour"):
    ...
    raise ValueError(
        "I could not find updatedPatJetsTransientCorrectedAK8DeepFlavour to embed the tagInfos, please check the cfg"
    )
...
process.patGenJetMatchWithNu = cms.EDProducer(
    ...
    src=cms.InputTag(
        "selectedUpdatedPatJetsDeepFlavour"
...
process.patGenJetMatchRecluster = cms.EDProducer(
    "GenJetMatcher",  # cut on deltaR; pick best by deltaR
    src=cms.InputTag(
        "selectedUpdatedPatJetsDeepFlavour"
    ),  # RECO jets (any View<Jet> is ok)
...
process.patGenJetsAK8MatchRecluster = cms.EDProducer(
    "GenJetMatcher",
...
process.patGenJetsAK8MatchWithNu = cms.EDProducer(
    "GenJetMatcher",
    src=cms.InputTag("selectedUpdatedPatJetsAK8DeepFlavour"),
...
process.patGenSubJetsMatchWithNu = cms.EDProducer(
    "GenJetMatcher",
...
process.patGenSubJetsMatchRecluster = cms.EDProducer(
    "GenJetMatcher",
    src=cms.InputTag("selectedUpdatedPatJetsSoftDropSubjetsDeepFlavour"),
...
loadWithPrefix(
    process, "RecoVertex.AdaptiveVertexFinder.inclusiveVertexing_cff", "looseIVF"
)
process.looseIVFinclusiveCandidateVertexFinder.primaryVertices = cms.InputTag(
    "offlineSlimmedPrimaryVertices"
)
process.looseIVFinclusiveCandidateVertexFinder.tracks = cms.InputTag(
    "packedPFCandidates"
)
...
process.looseIVFcandidateVertexArbitrator.primaryVertices = cms.InputTag(
    "offlineSlimmedPrimaryVertices"
)
process.looseIVFcandidateVertexArbitrator.tracks = cms.InputTag("packedPFCandidates")
process.looseIVFcandidateVertexArbitrator.secondaryVertices = cms.InputTag(
    "looseIVFcandidateVertexMerger"
)
...
process.deepntuplizer.LooseSVs = cms.InputTag(
    "looseIVFinclusiveCandidateSecondaryVertices"
)
...
process.deepntuplizerAK8.bDiscriminators.append(
