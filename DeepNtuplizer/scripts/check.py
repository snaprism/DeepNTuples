...
parse = ARgpsrs(
    "check the status of the jobs on the batch queue, pass a directory and an action. Action can be: none, remove, resubmit. The action remove removes failed output. After that, the failed jobs can be resubmitted with resubmit"
)

parse.add_argument("dirs", metavar="N", nargs="+", help="dirs to check")
...
for dir in dirs:
    ...
    if len(dir.split("/")) > 1:
        print(
            "please run this script directly in the parent directory of the job directory"
        )
        exit()
    ...
    for i in range(nJobs):
        ...
        if jobstatus_list[jobno] == "S" and not os.path.isfile(
            dir + "/output/" + dir + "_" + str(jobno) + ".root"
        ):
            jobstatus_list[jobno] = "F"
    ...
    if (action == "merge" or nsucc == nJobs or action == "filelist") and nsucc > 0:
        ...
        if action == "merge":
            ...
            for out in succoutfile:
                ...
                os.system(
                    "hadd " + outputroot + out
                )  # use subprocess for parallelisatio later
    ...
    if action == "clean":
        ...
        print(
            'if no jobs are running anymore, please run "check.py <dirs> --action clean" to clean temporary files'
        )
