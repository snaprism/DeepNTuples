...
def doSub():
    ...
    parser.add_argument(
        "--file", default="samples.cfg", help="file containing a sample list"
    )
    parser.add_argument("--nosubmit", default=False, help="no submission")
    parser.add_argument(
        "--outpath", default="", help="set path to store the .root output"
    )
    parser.add_argument(
        "--walltime", default="28800", help="set job wall time in seconds"
    )
    parser.add_argument(
        "--maxsize", default="15000", help="set maximum allowed size of output ntuple"
    )
    ...
    checkcert = subprocess.Popen(
        ["voms-proxy-info", "-path"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    sout, serr = checkcert.communicate()
    certpath = sout
    checkcert = subprocess.Popen(
        ["voms-proxy-info", "-timeleft"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    sout, serr = checkcert.communicate()
    certtime = sout
    ...
    if float(certtime) < 2 * 60 * 60:
        print(
            "grid proxy loses validity in less than 2 hours, please renew before job submission."
        )
        exit()
    ...
    if nousercertsfound:
        print(
            "please follow the Twiki https://twiki.cern.ch/twiki/bin/view/CMSPublic/WorkBookStartingGrid"
        )
        exit()
    ...
    configFile = os.path.realpath(
        os.path.join(args.jobdir, os.path.basename(configFile))
    )

    globalOutDir = (
        eosGlobalOutDir + "/" + time.strftime("%a_%H%M%S") + "_" + args.jobdir
    )
    ...
    for sampledescription in lines:
        ...
        if not isdasname:
            ...
            if not os.path.isfile(samplescriptdir + scriptfile + ".py"):
                ...
                dasquery = subprocess.Popen(
                    cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
                sout = dasquery.communicate()[0]
                filelist = [
                    '"%s",' % f.decode("UTF-8")
                    for f in sout.strip().split("\n".encode(encoding="UTF-8"))
                ]

                template_sample = os.path.join(
                    os.environ["CMSSW_BASE"],
                    "src/DeepNTuples/DeepNtuplizer/python/samples/samples_template.py",
                )
        ...
        nJobs = entries[0]
        totalfiles = len(cmssource.source.fileNames) + len(
            cmssource.source.secondaryFileNames
        )
        if int(nJobs) > totalfiles:
            print(
                "reduced number of jobs to number of files (",
                totalfiles,
                ") from ",
                nJobs,
            )
        ...
        if len(entries) > 3:
            jobargs = entries[3]
        jobpath = os.path.join(args.jobdir, outputFile)
...
then
   ...
   then
     ...
     xrdcp $OUTPUT*.root root://eosuser.cern.ch//eos/user/a/aljung/$NTUPLEOUTFILEPATH
   fi
   exitstatus=$?
   rm -f $OUTPUT*.root
   if [ $exitstatus != 0 ]
